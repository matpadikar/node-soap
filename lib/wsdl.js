...
MessageElement.prototype.postProcess = function(definitions,allDetails) {
  var part = null;
  var child;
    ...
    if (this.element.$type) {
      ...
      if (typeNs) {
        if (type.name in Primitives) {
          ...
          if (ctype) {
            this.parts = ctype.description(definitions, schema.xmlns,allDetails);
          }
        }
      }
    }
    else {
      var method = this.element.description(definitions, schema.xmlns,allDetails);
...
OperationElement.prototype.postProcess = function(definitions, tag,allDetails) {
  var children = this.children;
  for (var i = 0, child; child = children[i]; i++) {
    ...
    message.postProcess(definitions,allDetails);
    if (message.element) {
...
PortTypeElement.prototype.postProcess = function(definitions,allDetails) {
  var children = this.children;
  ...
  for (var i = 0, child; child = children[i]; i++) {
    if (child.name !== 'operation')
      continue;
    child.postProcess(definitions ,'portType',allDetails);
...
BindingElement.prototype.postProcess = function(definitions,allDetails) {
  var type = splitQName(this.$type).name,
    portType = definitions.portTypes[type],
    style = this.style,
    children = this.children;
  if (portType){
    portType.postProcess(definitions,allDetails);
    this.methods = portType.methods;

    for (var i = 0, child; child = children[i]; i++) {
      if (child.name !== 'operation')
        continue;
      child.postProcess(definitions, 'binding',allDetails);
      children.splice(i--, 1);
...
ServiceElement.prototype.postProcess = function(definitions,allDetails) {
  var children = this.children,
  ...
  if (children && children.length > 0) {
    for (var i = 0, child; child = children[i]; i++) {
      ...
      if (binding) {
        binding.postProcess(definitions,allDetails);
...
SimpleTypeElement.prototype.description = function(definitions,allDetails) {
  var children = this.children;
  for (var i = 0, child; child = children[i]; i++) {
...
RestrictionElement.prototype.description = function(definitions, xmlns,allDetails) {
  var children = this.children;
  var desc;
  for (var i=0, child; child=children[i]; i++) {
    if (child instanceof SequenceElement ||
            child instanceof ChoiceElement) {
      desc = child.description(definitions, xmlns,allDetails);
      break;
    }
  }
  if (desc && this.$base) {
    ...
    desc.getBase = function() {
      return typeElement.description(definitions, schema.xmlns,allDetails);
    };
    return desc;
  }
...
ExtensionElement.prototype.description = function(definitions, xmlns,allDetails) {
  var children = this.children;
  var desc = {};
  for (var i=0, child; child=children[i]; i++) {
    if (child instanceof SequenceElement ||
  ...
  if (this.$base) {
    ...
    if (typeName in Primitives) {
      ...
      if (typeElement) {
        var base = typeElement.description(definitions, schema.xmlns,allDetails);
...
ComplexTypeElement.prototype.description = function(definitions, xmlns,allDetails) {
  var children = this.children || [];
  for (var i=0, child; child=children[i]; i++) {
    if (child instanceof ChoiceElement ||
      ...
      return child.description(definitions, xmlns,allDetails);
    }
  }
  return {};
};

ComplexContentElement.prototype.description = function(definitions, xmlns,allDetails) {
  var children = this.children;
  for (var i = 0, child; child = children[i]; i++) {
...
SimpleContentElement.prototype.description = function(definitions, xmlns,allDetails) {
  var children = this.children;
  for (var i = 0, child; child = children[i]; i++) {
    if (child instanceof ExtensionElement) {
      return child.description(definitions, xmlns,allDetails);
...
ElementElement.prototype.description = function(definitions, xmlns,allDetails) {
  ...
  if (type) {
    ...
    if (ns && definitions.schemas[ns]) {
      ...
      if(allDetails){
        ...
        for(let key in this){
          if(key.startsWith("$")){
            if(key!=="$name")
              propertyDetails[key.substring(1)] = this[key];
          }
    ...
    if (typeElement && !(typeName in Primitives)) {
      ...
      if (!(typeName in definitions.descriptions.types )) 
      {
        ...
        var description = typeElement.description(definitions, xmlns,allDetails);
        ...
        if (this.$ref) {
          ...
          if(allDetails){
            element[name][typeName] = definitions.descriptions.types[typeName];
          }
          else
   ...
   if(!allDetails){
    ...
    for (var i = 0, child; child = children[i]; i++) {
      if (
        ...
        element[name] = child.description(definitions, xmlns,allDetails);
      }
    }
  }
  return element;
...
SequenceElement.prototype.description = function(definitions, xmlns,allDetails) {
  var children = this.children;
  var sequence = {};
  for (var i = 0, child; child = children[i]; i++) {
    ...
    var description = child.description(definitions, xmlns,allDetails);
...
ChoiceElement.prototype.description = function(definitions, xmlns,allDetails) {
  var children = this.children;
  var choice = {};
  for (var i=0, child; child=children[i]; i++) {
    var description = child.description(definitions, xmlns,allDetails);
...
MessageElement.prototype.description = function(definitions,allDetails) {
  if (this.element) {
    return this.element && this.element.description(definitions,allDetails);
  }
  var desc = {};
...
PortTypeElement.prototype.description = function(definitions,allDetails) {
  var methods = {};
  for (var name in this.methods) {
    var method = this.methods[name];
    methods[name] = method.description(definitions,allDetails);
  }
  return methods;
};

OperationElement.prototype.description = function(definitions,allDetails) {
  var inputDesc = this.input ? this.input.description(definitions,null,allDetails) : null;
  var outputDesc = this.output ? this.output.description(definitions,null,allDetails) : null;
  return {
...
BindingElement.prototype.description = function(definitions,allDetails) {
  var methods = {};
  for (var name in this.methods) {
    var method = this.methods[name];
    methods[name] = method.description(definitions,allDetails);
...
ServiceElement.prototype.description = function(definitions,allDetails) {
  var ports = {};
  for (var name in this.ports) {
    var port = this.ports[name];
    ports[name] = port.binding.description(definitions,allDetails);
  }
  return ports;
};
    ...
    self.processIncludes(function(err) {
      ...
      if (services) {
        for (name in services) {
          services[name].postProcess(self.definitions,self.options.allDetails);
        }
      }
...
WSDL.prototype._initializeOptions = function (options) {
  ...
  this.options.allDetails = options.allDetails;
...
WSDL.prototype.describeServices = function() {
  ...
  for (var name in this.services) {
    ...
    services[name] = service.description(this.definitions, this.options.allDetails);
  }
  return services;
};
